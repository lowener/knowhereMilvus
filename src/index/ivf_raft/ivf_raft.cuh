    ...
    Search(const DataSet& dataset, const Config& cfg, const BitsetView& bitset) const override {
        ...
        try {
            ...
            for (auto row_index = 0; row_index < gpu_results.ids().extent(0); ++row_index) {
                raft::copy(ids.get() + row_index * ivf_raft_cfg.k,
                           gpu_results.ids_data() + gpu_results.ids().extent(1) * row_index,
                           ivf_raft_cfg.k,
                           res_->get_stream().value());
                raft::copy(dis.get() + row_index * ivf_raft_cfg.k,
                           gpu_results.dists_data() + gpu_results.dists().extent(1) * row_index,
                           ivf_raft_cfg.k,
                           res_->get_stream().value());
            }
